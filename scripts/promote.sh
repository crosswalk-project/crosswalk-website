...
Exiting. You will need to manually edit '${file}'. When done, re-run:
...
function update_version_string () {
    ...
    cat << EOF | perl -077 -pe "${pattern}" > $file
/*
...
function run () {
    ...
    if [[ -e "tmp-${live}" ]]; then
        while true; do
            echo -n "tmp-${live} already exists. Use current version? [Yn] "
            read answer
            case $answer in
            N|n)
                rm -rf "tmp-${live}" || die "Unable to remove directory."
                break
                ;;
            ""|Y|y)
                command="continue"
    ...
    STAGE_GIT="--git-dir=.git --work-tree=tmp-${live}"
